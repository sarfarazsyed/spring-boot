...
public class CustomGlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<String> springHandleDuplicate(UserAlreadyExistsException u, WebRequest response) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(u.getMessage());
    }

    @ExceptionHandler(UnSupportedFieldPatchException.class)
    public ResponseEntity<String> springUnSupportedFieldPatch(UnSupportedFieldPatchException u, WebRequest response) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(u.getMessage());
    }
